            ...
            public void onDataChange(@NonNull DataSnapshot snapshot) {
                ...
                for (int i = 0; i < snapshot.child("qsms").getChildrenCount(); i++) {
                    ...
                    for (int j = 0; j < snapshot.child("qsms/" + (i + 1) + "/scools").getChildrenCount(); j++) {
                        ...
                        int l = Integer.parseInt(snapshot.child("qsms/" + (i + 1) + "/scools/" + (j + 1) + "/from").getValue().toString()); int e =  Integer.parseInt(snapshot.child("qsms/" + (i + 1) + "/scools/" + (j + 1) + "/to").getValue().toString());
                        for (; l <= e; l++) {
